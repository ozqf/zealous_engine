...
internal char g_baseDir[Z_MAX_PATH];
internal char g_appDir[Z_MAX_PATH];
...
internal ErrorCode WinIO_StageFile(const char *path, i32 bOnlyPacks, ZEBuffer *result)
{
    ...
    else
    {
        ...
        ZE_BUILD_STRING(diskPath, Z_MAX_PATH, "%s/%s", g_baseDir, path);
        FILE* f;
        errno_t err;
        err = fopen_s(&f, diskPath, "rb");
        if (f == NULL)
        {
...
extern "C" ZEFileIO WinIO_Init(const char *baseDir, const char *appDir)
{
    ...
    ZStr_CopyLimited(baseDir, g_baseDir, Z_MAX_PATH);
    ZStr_CopyLimited(appDir, g_appDir, Z_MAX_PATH);
    printf("WIN IO init. dirs: base \"%s\" and app \"%s\"\n",
        g_baseDir, g_appDir);
    // data files are scanned in reverse order
    WinIO_GatherDataFiles(g_baseDir);
    if (ZStr_Compare(g_baseDir, g_appDir) == 0)
    {
        g_appDir[0] = '\0';
    }
    else
    {
        WinIO_GatherDataFiles(g_appDir);
    }
